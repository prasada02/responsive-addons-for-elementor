...
jQuery( document ).ready(
    ...
    if (
      ...
      $(".responsive-addons-for-elementor-tabs-inner-content").addClass(
        "responsive-addons-for-elementor-widgets-section-spacing"
      );
    }
    if (hash === "#widgets") {
      ...
      $(".responsive-addons-for-elementor-tab").removeClass(
        "responsive-addons-for-elementor-active-tab"
      );
      $(".responsive-addons-for-elementor-widgets-tab").addClass(
        "responsive-addons-for-elementor-active-tab"
      );
      $("#rael_widgets").show();
    }
    if (hash === "#templates") {
      goToRST();
      $(".responsive-addons-for-elementor-tabs-inner-content").css(
        "background-image",
        "url('" + localize.raelurl + "admin/images/rst-template-preview.jpg')"
      );
    }
    $(".responsive-addons-for-elementor-tab-content").hide();
    $(".responsive-addons-for-elementor-tab").removeClass(
    ...
    $(
      ".responsive-addons-for-elementor-" + hash.substring(1) + "-tab"
    ).addClass("responsive-addons-for-elementor-active-tab");
    $("#rael_" + hash.substring(1)).show();

    $(".responsive-addons-for-elementor-tab").click(function () {
      $(".responsive-addons-for-elementor-tab-content").hide();
      $(".responsive-addons-for-elementor-tab").removeClass(
        "responsive-addons-for-elementor-active-tab"
      );
    ...
    $(window).on("hashchange", function () {
      ...
      if (currentHash === "#widgets" || currentHash === "#settings") {
        ...
        $(".responsive-addons-for-elementor-tabs-inner-content").removeClass(
          "responsive-addons-for-elementor-widgets-section-spacing"
        );
      }
      if (currentHash === "#settings") {
        ...
        $(".responsive-addons-for-elementor-settings-tab").addClass(
          "responsive-addons-for-elementor-active-tab"
        );
        $("#rael_settings").show();
      }
    ...
    $(".rael-widgets-input-checkbox").on("change", function (event) {
      let index = $(this).data("index");
      let value = $(this).prop("checked");
      let widgetStatus = true;
      $(".rael-widget-category-card").each(function () {
        let widgetSwitchValue = $(this)
          .find(".rael-widgets-input-checkbox")
          .prop("checked");
    ...
    $("#rael-widgets-toggle-widgets").on("change", function (event) {
    ...
    $(".rael-widget-tab").on("click", function (event) {
    ...
    $("#rael-input-search-widgets").on("keyup", function (event) {
      let query = $(this).val();
      ...
      if (query === "") {
        ...
        $(".rael-widget-category-card").each(function () {
          let widgetTitle = $(this)
            .find(".rael-widgets-card-title p")
            .text()
            .toLowerCase();
    ...
    function goToRST() {
      if (localize.isRSTActivated) {
        window.location.href =
          localize.siteurl + "/wp-admin/admin.php?page=responsive_add_ons";
        return;
      }
    }
    ...
    $("#rael_settings_save_changes").on("click", function (event) {
      ...
      let gmapLocalizationLang = $(
        "#rael_google_map_settings_localization_language"
      ).val();
    ...
    // Open popup on settings icon click for RAE Duplicator extension
    ...
    $("#rael-popup-save").on("click", function () {
      ...
      function updateDuplicatorSettingsState() {
        const isEnabled = $("#duplicator").is(":checked");

        if (!isEnabled) {
          $('.rael-settings-trigger[data-widget="duplicator"]')
            .addClass("disabled")
            .css({
              "pointer-events": "none",
              color: "#afafaf",
      ...
      updateDuplicatorSettingsState();

      // Update when the toggle is clicked
      $(document).on("change", "#duplicator", function () {
        updateDuplicatorSettingsState();
      });
  }
);