...
const InstallButton = ({ type, status, nonce, redirect, buttonText, setButtonText, slug }) => {
    const handleInstall = (e) => {
        e.preventDefault();
        setButtonText(__('Installing...', 'responsive-addons-for-elementor'));
        ...
        installFunction({
            slug: slug,
            success: function () {
                setButtonText(__('Activating...', 'responsive-addons-for-elementor'));
                activatePlugin(nonce, redirect, setButtonText);
            },
            error: function (error) {
                console.error(`${type} installation failed:`, error);
                setButtonText(__('Install Failed', 'responsive-addons-for-elementor'));
            }
        });
    };
        ...
        case 'activated':
            return (
                <button className="mt-1.125 py-2.5 px-3.5 border border-blue-300 text-blue-300 bg-white rounded-md text-sm leading-5 font-medium capitalize cursor-not-allowed" disabled>
                    {__( 'Activated', 'responsive-addons-for-elementor' )}
                </button>
            );
        default:
            return (
