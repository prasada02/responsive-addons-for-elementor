...
if(-1!==devices.indexOf('mobile')){RaelSticky.getStickySectionsMobile.push($scope)}},makeStickyOnScroll:function makeStickyOnScroll(element){var $el=$(element);this.placeholder=$('<div>').height($el.outerHeight()).hide();var self=this;this.scrollHandler=function scrollHandler(){var scrollTop=$(window).scrollTop();if(scrollTop>=self.originalOffsetTop&&!self.isSticky){$el.after(self.placeholder.show());$el.css({position:'fixed',top:0,width:$el.outerWidth()+'px',zIndex:1100});self.isSticky=!0}else if(scrollTop<self.originalOffsetTop&&self.isSticky){self.removeStickyStyles()}};$(window).on('scroll.raelSticky-'+this.getUniqueID(),this.scrollHandler)},removeStickyStyles:function removeStickyStyles(){var $el=$(this.target[0]);$el.css({position:'',top:'',width:'',zIndex:''});if(this.placeholder){this.placeholder.hide()}
