var WooCheckoutMain = function WooCheckoutMain( $scope, $ )
{
	var $rael_woo_checkout_main = $scope.find( ".rael-woo-checkout" ).eq( 0 );
	...
	var button_prev = $( '.rael-woo-checkout-btn-prev' ),
		button_next = $( '.rael-woo-checkout-btn-next' ),
		first_step = 0,
		last_step = 0,
		active_step = 1;
	if ( $rael_woo_checkout_main.hasClass( "layout-split" ) )
	{
	...
	else if ( $rael_woo_checkout_main.hasClass( "layout-multi-steps" ) )
	{
	...
	var button_prev = $( '.rael-woo-checkout-btn-prev' );
	var button_next = $( '.rael-woo-checkout-btn-next' );
	var first_step = 0;
	var last_step = 0;
	var active_step = 1;
	...
	{
		var topPosition = $( '.rael-woo-checkout' );
		if ( topPosition )
		{
	...
	{
		$( '.rael-wrapper .woocommerce-NoticeGroup-checkout, .rael-wrapper .woocommerce-error, .rael-wrapper .woocommerce-message, .woocommerce .woocommerce-error' ).remove();
	}
	...
	function validate_step_fields( active_step )
	{
		...
		if ( active_section )
		{
			var all_inputs = active_section.find( ":input" ).not( '.rael-disabled-field, .woocommerce-validated,:hidden' );
			...
			$.each( all_inputs, function ( field )
			{
				...
				if ( parent.hasClass( 'validate-required' ) )
				{
					...
					if ( isEmpty( value ) )
					{
						...
						{
							getBillingOrShippingText = localize.rael_translate_text.billing_text;
						}
						var singleErrorMsgTxt = liTagStart + getBillingOrShippingText + ' ' + label + localize.rael_translate_text.required_text + liTagEnd;
					...
					else if ( parent.hasClass( 'validate-email' ) )
					{
						...
						if ( !valid_email )
						{
							...
							var getBillingOrShippingText = name.split( '_' );
							getBillingOrShippingText = getBillingOrShippingText[ 0 ];
							var singleErrorMsgTxt = liTagStart + localize.rael_translate_text.invalid_text + getBillingOrShippingText + ' ' + label + liTagEnd;
							errorMsgTxt += singleErrorMsgTxt;
							valid = false;
							scrol_to_error();
						}
					...
					else if ( parent.hasClass( 'validate-postcode' ) )
					{
						...
						$.ajax(
						{
							...
							data:
							{
								action: 'rael_woo_checkout_post_code_validate',
								data:
								{
									country: country,
									postcode: value
	...
	if ( tabs_wrapper )
	{
		tabs_wrapper.on( 'click', '#rael_place_order', function ()
		{
...
{
	elementorFrontend.hooks.addAction( "frontend/element_ready/rael-woocommerce-checkout.default", WooCheckoutMain );
} );
