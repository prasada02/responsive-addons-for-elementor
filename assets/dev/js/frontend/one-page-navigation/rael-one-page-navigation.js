...
var OnePageNavHandler = function OnePageNavHandler($scope, $) {
	  var onepage_nav_elem = $scope.find(".rael-one-page-nav").eq(0);
  var $section_id = "#" + onepage_nav_elem.data("section-id"),
	  ...
	  $target_dot = $section_id + " .rael-one-page-nav-item a",
	  $nav_item = $section_id + " .rael-one-page-nav-item",
	  $active_item = $section_id + " .rael-one-page-nav-item.active";
  $($target_dot).on("click", function (e) {
	...
	$($section_id + " .rael-one-page-nav-item").removeClass("active");
  ...
  function updateDot() {
		$(".elementor-section").each(function () {
		  ...
		  if ($this.offset().top - $(window).height() / 2 < $(window).scrollTop() && ($this.offset().top >= $(window).scrollTop() || $this.offset().top + $this.height() - $(window).height() / 2 > $(window).scrollTop())) {
			$($section_id + ' .rael-one-page-nav-item a[data-row-id="' + $this.attr("id") + '"]').parent().addClass("active");
	  } else {
			$($section_id + ' .rael-one-page-nav-item a[data-row-id="' + $this.attr("id") + '"]').parent().removeClass("active");
	  }
	});
  }
  if ($scroll_wheel == "on") {
	...
	if ($scroll_touch == "on") {
		...
		if (touches.pointerType === "touch" || e.type === "touchend") {
		  ...
		  var deltaY = startY - Y;
		  var time = touches.timeStamp - timestamp; // swipe up.
		  if (deltaY < 0) {
				if ($($active_item).prev().length > 0) {
				  $($active_item).prev().find("a").trigger("click");
			}
...
$(window).on("elementor/frontend/init", function () {
	  elementorFrontend.hooks.addAction("frontend/element_ready/rael-one-page-nav.default", OnePageNavHandler);
});
})(jQuery);
